...
const Login = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      await fetch(
        "https://warehouse-management-serve-production.up.railway.app/api/todo/gettoken",
        {
          method: "POST",
          headers: {
  ...
  return (
    <div>
      ...
      <span className=" text-sm cursor-pointer my-3 block" href="/register">
        ...
        <NavLink className="text-blue-500" to="/register/signup">
          Sign Up Free!
        </NavLink>
      </span>
      <form
      ...
      >
        ...
        <span className="flex items-center justify-center mt-5">
          <GoogleButton onClick={googleLoginFacebook} />
        </span>
      </form>
    </div>
  );
